...
/**
  * @brief This is the main class used to create a session to run a model.
  * Sample simple usage:
  *  CPUExecutionProviderInfo epi;
  *  ProviderOption po{"CPUExecutionProvider", epi};
  *  SessionOptions so(vector<ProviderOption>{po});
  *  auto session_object = Session::Create(so);
...
class Session {
 public:
  /**
   * The type of session, must be specified when Create().
   */
  ...
  /**
    ...
    * @return OK if success.
    */
  virtual common::Status RegisterGraphTransformer(std::unique_ptr<onnxruntime::GraphTransformer> p_graph_transformer) = 0;
  ...
  * @return OK if success
  */
  virtual common::Status LoadCustomOps(const std::vector<std::string>& dso_list) = 0;
  ...
  /**
    ...
    * @param feed_names the names of the inputs owned by client code and should not be changed during
  ...
  */
  ...
  /**
    * Start profiling on this session. This simply turns on profiling events to be 
